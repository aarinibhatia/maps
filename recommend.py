...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    mean_lat = mean([restaurant_location(x)[0] for x in cluster])
    mean_long = mean([restaurant_location(y)[1] for y in cluster])
...
def find_predictor(user, restaurants, feature_fn):
    ...
    s_xy = sum([(x - mean(xs))*(y - mean(ys)) for x, y in zipped])
    b = s_xy/s_xx
    a = mean(ys)-b*mean(xs)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    fn = max(feature_fns, key = lambda feature: find_predictor(user, reviewed, feature)[1])
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 9
    revd_dict = {restaurant_name(revd_rest): user_rating(user, restaurant_name(revd_rest)) for revd_rest in reviewed}
    revd_dict.update({restaurant_name(pred_rest): predictor(pred_rest) for pred_rest in restaurants if pred_rest not in reviewed})
    return revd_dict
    # END Question 9
